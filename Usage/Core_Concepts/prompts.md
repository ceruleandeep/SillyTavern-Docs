...
### Adjusting the Main Prompt with Conversation History

When adjusting the main prompt to improve the AI's responses, consider the effect of conversation history. The AI picks up a lot from the conversation history. The history is its memory of past events, character interactions and relationships, and its style guide for word choice and writing style.

* Use this to your advantage by providing [example messages](characterdesign.md#examples-of-dialogue) showing how you want the AI to respond. Showing the AI what you want is often easier than trying to explain it!

* If you already have an established conversation history, changing the main prompt has a limited effect on the AI's responses. In terms of events and relationships, the AI assumes that the main prompt occurred in the distant past, and the message history updates events. In terms of writing style and word choice, the AI assumes that all the messages in history were generated in accordance with the rules in the *current* main prompt, and that it should continue to generate messages in the same way. Some suggestions for dealing with this are:

  * insert current instructions closer to the end of message history, for example by using an [Author's Note](Author's-Note.md)
